...
server <- function(input, output, session) {
  ...
  dataset_df_reactive <- reactiveVal(dataset_df) # Initialize as NULL
  ...
  output$table <- renderDT({
    datatable(filtered_data(),filter = 'top', options = list(pageLength = 5))
  })
  ...
  #
  # # Store dataset in a reactive value
  # ct_dataset_df_reactive <- reactiveVal(NULL)  # Initialize as NULL
  #
  # # Observe when both inputs change and fetch data
  ...
  #     })
  #
  #     ct_dataset_df_reactive(ct_codelist_df)
  #
  #     #print(json_list)  # Debugging: Print API response
  #   } else {
  #     print(paste("API request failed with status:", resp_status(ctresp)))
  #     ct_dataset_df_reactive(NULL)  # Reset the dataset on failure
  #   }
  # })
  #
  #
  # # Reactive filtered data based on selections
  # ct_filtered_data <- reactive({
  #   req(ct_dataset_df_reactive())  # Ensure data exists before returning
  #   ct_dataset_df_reactive()
  # })
  #
  ...
  output$ct_table <- renderDT({
    ...
    datatable(ct_react_filtered_data(), filter = 'top', options = list(pageLength = 5))
  })
      ...
      tryCatch(
        {
          ...
          filtered_data <- ct_codelist_df %>% filter(eval(parse(text = filter_expr)))
